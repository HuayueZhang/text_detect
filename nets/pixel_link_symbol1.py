...
#             target_shape = target_layer.shape[1:-1] # NHWC
    ...
    def _fuse_by_cascade_upsample_sum_conv1x1_deconv2x2(self, num_classes, scope):
        ...
        with tf.variable_scope(scope):
            ...
            # base_shape = tf.constant((128, 128), dtype=tf.int32)
            ...
            with slim.arg_scope(self.arg_scope):
                ...
                # version 1.1---------------
                # layer = slim.conv2d(smaller_layer, 256, [1, 1], scope='conv1x1_1',
                #                     normalizer_fn=slim.batch_norm, normalizer_params=batch_norm_params)
                # layer = tf.image.resize_images(layer, 2 * base_shape)
                #
                # layer = slim.conv2d(layer, num_classes, [1, 1], scope='conv1x1_2',
                #                     normalizer_fn=slim.batch_norm, normalizer_params=batch_norm_params)
                # layer = tf.image.resize_images(layer, 4 * base_shape)
                # version 1.2---------------
                layer = slim.conv2d(smaller_layer, 256, [1, 1], scope='conv1x1_1',
                                    normalizer_fn=slim.batch_norm, normalizer_params=batch_norm_params)
                # layer = slim.conv2d_transpose(layer, 256, [2, 2], scope='deconv2x2_1') # slim.con2d_transpose不知道如何控制输出尺寸
                input_shape = [layer.get_shape()[i].value for i in range(4)]
                output_shape = [input_shape[0], 2*input_shape[1], 2*input_shape[2], input_shape[3]]
                layer = dssd_vgg.conv2d_transpose(layer, output_shape, [2, 2], scope='deconv2x2_1')

                layer = slim.conv2d(layer, 256, [1, 1], scope='conv1x1_2',
                                    normalizer_fn=slim.batch_norm, normalizer_params=batch_norm_params)
                # layer = slim.conv2d_transpose(layer, num_classes, [2, 2], scope='deconv2x2_2')
                input_shape = [layer.get_shape()[i].value for i in range(4)]
                output_shape = [input_shape[0], 2*input_shape[1], 2*input_shape[2], num_classes]
                layer = dssd_vgg.conv2d_transpose(layer, output_shape, [2, 2], scope='deconv2x2_2')
