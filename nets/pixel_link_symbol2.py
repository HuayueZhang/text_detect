...
MODEL_TYPE_vgg16_DSSD = 'vgg16_dssd'
MODEL_TYPE_vgg16_DSSD_SSD = 'vgg16_dssd_ssd'
MODEL_TYPE_vgg16_no_dilation = 'vgg16_no_dilation'

FUSE_TYPE_cascade_upsample_sum_conv1x1_deconv2x2 = 'cascade_upsample_sum_conv1x1_deconv2x2'
...
class PixelLinkNet(object):
    def __init__(self, inputs, is_training):
        ...
        self._build_network()
        self._fuse_feat_layers_for_pixel_link()
        self._ssd_pred_module()
        self._logits_to_scores()


    def _build_network(self):
        import config
        if config.model_type == MODEL_TYPE_vgg16_DSSD:
            from nets import dssd_vgg
            with slim.arg_scope([slim.conv2d],
                ...
                with slim.arg_scope([slim.conv2d, slim.max_pool2d],
                                    padding='SAME') as sc:
                    self.arg_scope = sc
                    self.net, self.end_points = dssd_vgg.basenet(
                              inputs =  self.inputs)

        elif config.model_type == MODEL_TYPE_vgg16_DSSD_SSD:
            from nets import dssd_vgg
            with slim.arg_scope([slim.conv2d],
                        activation_fn=tf.nn.relu,
                        weights_regularizer=slim.l2_regularizer(config.weight_decay),
...
#             target_shape = target_layer.shape[1:-1] # NHWC
    ...
    def _fuse_by_cascade_conv1x1_128_upsamle_concat_conv1x1_2(self, scope, num_classes = 32):
        ...
        with tf.variable_scope(scope):
            ...
            for idx in range(0, len(config.feat_layers))[::-1]: #[4, 3, 2, 1, 0]
                ...
                current_score_map = self._score_layer(current_layer, num_classes, current_layer_name)
                if smaller_score_map is None:
                    smaller_score_map = current_score_map
                else:
                    upscore_map = self._upsample_layer(smaller_score_map, current_score_map)
    ...
    def _fuse_by_cascade_conv1x1_upsample_sum(self, num_classes, scope):
        ...
        with tf.variable_scope(scope):
            ...
            for idx in range(0, len(config.feat_layers))[::-1]: #[4, 3, 2, 1, 0]
                current_layer_name = config.feat_layers[idx]
                current_layer = self.end_points[current_layer_name]        # extract different layers from net
                current_score_map = self._score_layer(current_layer,       # transform the layers from net into the format of scores,
                                      num_classes, current_layer_name)     # 2 channels for pixels cls and 16 channels for links cls
                if smaller_score_map is None:                              # by a conv_2d layer
                    smaller_score_map = current_score_map
                else:
                    upscore_map = self._upsample_layer(smaller_score_map, current_score_map)  # default bilinear interpolation
                    smaller_score_map = current_score_map + upscore_map
            
        return smaller_score_map

    def _fuse_by_cascade_upsample_sum_conv1x1_deconv2x2(self, num_classes, scope):
        """
        Zhanghuayue follow the segmentation net structure of
        ...
        with tf.variable_scope(scope):
            smaller_layer = None
            for idx in range(0, len(config.feat_layers)):
                current_layer_name = config.feat_layers[idx]
                current_layer = self.end_points[current_layer_name]
                if smaller_layer is None:
                    smaller_layer = current_layer
                else:
                    upsample_layer = self._upsample_layer(smaller_layer, current_layer)
                    smaller_layer = current_layer + upsample_layer

            base_shape = tf.shape(smaller_layer)[1: -1] ######################################
            ...
            with slim.arg_scope(self.arg_scope):
                ...
                layer = slim.conv2d(smaller_layer, 128, [1, 1], stride=1, scope='conv1x1_1',
                                    normalizer_fn=slim.batch_norm, normalizer_params=batch_norm_params)
                layer = tf.image.resize_images(layer, 2 * base_shape)

                layer = slim.conv2d(layer, num_classes, [1, 1], stride=1, scope='conv1x1_2',
                                    normalizer_fn=slim.batch_norm, normalizer_params=batch_norm_params)
                layer = tf.image.resize_images(layer, 4 * base_shape)

        return layer


    def _fuse_feat_layers_for_pixel_link(self):
        import config
        if config.feat_fuse_type == FUSE_TYPE_cascade_conv1x1_upsample_sum:
            self.pixel_cls_logits = self._fuse_by_cascade_conv1x1_upsample_sum(    # net layers => conv2d => pixel cls logits
                config.num_classes, scope = 'pixel_cls')
            
            self.pixel_link_logits = self._fuse_by_cascade_conv1x1_upsample_sum(   # net layers => conv2d => pixel link cls logits
                config.num_neighbours * 2, scope = 'pixel_link')
            
        elif config.feat_fuse_type == FUSE_TYPE_cascade_upsample_sum_conv1x1_deconv2x2:
            self.pixel_cls_logits = self._fuse_by_cascade_upsample_sum_conv1x1_deconv2x2(
                config.num_classes, scope='pixel_cls')

            self.pixel_link_logits = self._fuse_by_cascade_upsample_sum_conv1x1_deconv2x2(
                config.num_neighbours * 2, scope='pixel_link')
    ...
    def _ssd_multibox_layer(self, inputs,
                           num_classes,
                           sizes,
                           ratios=[1],
                           normalization=-1,
        ...
        with slim.arg_scope(self.arg_scope):
            ...
            with tf.variable_scope('residual'):
                ...
                net1 = slim.stack(net, slim.conv2d, [(256, [1, 1]), (256, [1, 1]), (1024, [1, 1])], scope='conv_res')
            ...
            cls_pred = tf.reshape(cls_pred,
                                  ssd_vgg_300.tensor_shape(cls_pred, 4)[:-1] + [num_anchors, num_classes])

        # 网络得出的结果，是通过卷积的形式，使得(w*h)feature layer，每个点有4个anchors，输出(w*h*4*4)或(w*h*4*21)的数据结果
        return cls_pred, loc_pred


    def _ssd_pred_module(self):
        # Prediction and localisations layers.
        import config
        self.anchor_classes = []
        self.anchor_logits = []
        self.anchor_localisations = []
        for i, layer in enumerate(config.feat_layers):
            with tf.variable_scope(layer + '_box'):
                p, l = self._ssd_multibox_layer(self.end_points[layer],
                                                config.num_classes_ssd,
                                                config.anchor_sizes[i],
                                                config.anchor_ratios[i],
                                                config.normalizations[i])
            self.anchor_classes.append(slim.softmax(p))   # soft_max
            self.anchor_logits.append(p)
            self.anchor_localisations.append(l)
    ...
    def _logits_to_scores(self):
        ...
        import config
        self.pixel_cls_scores = self.pixel_cls_logits
...
#         shape = self.pixel_link_logits.shape.as_list()
        ...
        self.pixel_link_scores = self.pixel_link_logits
    ...
    def build_loss(self, pixel_cls_labels, pixel_cls_weights, 
                        ...
                        anchor_gclasses, anchor_glocalisations, anchor_gscores,
                        do_summary = True):
        """
        ...
        with tf.name_scope('pixel_link_loss'):
            ...
            pixel_link_loss = pixel_pos_link_loss + pixel_neg_link_loss * pixel_link_neg_loss_weight_lambda
            tf.add_to_collection(tf.GraphKeys.LOSSES, pixel_link_loss_weight * pixel_link_loss)


        with tf.name_scope('ssd_loss'):
            from ssd import train_ssd_network as ssd_op
            ssd_cls_loss, ssd_loc_loss = ssd_op.ssd_loss(self.anchor_logits,
                                                         self.anchor_localisations,
                                                         anchor_gclasses,
                                                         anchor_glocalisations,
                                                         anchor_gscores)
            tf.add_to_collection(tf.GraphKeys.LOSSES, ssd_cls_loss + 2 * ssd_loc_loss)
        ...
        if do_summary:
            ...
            tf.summary.scalar('ssd_cls_loss', ssd_cls_loss)
            tf.summary.scalar('ssd_loc_loss', ssd_loc_loss)